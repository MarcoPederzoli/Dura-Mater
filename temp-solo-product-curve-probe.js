"use strict";
/**
 * Curva solitario PRODOTTO:
 *  N=3..5: legacy + mano N
 *  N=6..8: virtual-multi + mano 2N
 * Uso: node temp-solo-product-curve-probe.js [seeds=40] [workers=7]
 *      node temp-solo-product-curve-probe.js 100 7   # sample piu' solido su tutto
 */
...
async function main() {
  const jobs = [];
  for (let n = 3; n <= 8; n++) {
    for (let seed = 0; seed < SEEDS; seed++) jobs.push({ n, seed });
  }
  console.log(
    "Solo PRODOTTO · N3-5: legacy+manoN · N6-8: vm+mano2N · refill ON · seeds=" +
      SEEDS +
      " · deals=" +
      jobs.length +
      " · workers=" +
  ...
  const rows = await runWorkerPool(
    path.join(__dirname, "temp-solo-product-curve-worker.js"),
    jobs,
    {
      workers: WORKERS,
      onProgress: (d, t) => {
  ...
  for (let n = 3; n <= 8; n++) {
    ...
    if (!b) continue;
    const tot = n * n;
    const cfg = n <= 5 ? "legacy+manoN" : "vm+mano2N";
    console.log(
      String(n).padEnd(4) +
        cfg.padEnd(18) +
  ...
  for (let n = 3; n <= 8; n++) {
    ...
    if (!b) continue;
    console.log(
      n + "x" + n + " wins=" + (b.winSeeds.slice(0, 15).join(",") || "-")
    );
  }
