"use strict";
/**
 * 6x6 con tutti i G legali >= 6 (G = 6 .. 12, mano >= 3).
 */
...
acquireHeavyProbeLock("6x6-ge6", argvRaw);
...
const N = 6;
const G_MIN = 6;
const G_MAX = Math.min(2 * N, Math.floor((N * N) / 3)); // 12
...
console.log(
  `\n6x6 G=${G_MIN}..${G_MAX} · seeds=${SEEDS} · ${tasks.length} deals · workers=${WORKERS}/${logicalCpuCount()}\n`
);

async function main() {
  const started = Date.now();
  const rows = await runWorkerPool(path.join(__dirname, "temp-6x6-ge6-worker.js"), tasks, {
    workers: WORKERS
  });
  ...
  console.log("-".repeat(52));
  console.log(`OVERALL 6x6 G>=6: ${((100 * W) / D).toFixed(1)}% (${W}/${D})`);
  if (fails.length) {
    console.log(`\nFAILS (${fails.length}):`);
