"use strict";
/**
 * 8x8 con tutti i G legali >= 8 (G = 8 .. max mano>=3).
 */
...
acquireHeavyProbeLock("8x8-ge8", argvRaw);
...
const N = 8;
const G_MIN = 8;
const G_MAX = Math.min(2 * N, Math.floor((N * N) / 3)); // min(16, 21) = 16
...
console.log(
  `\n8x8 G=${G_MIN}..${G_MAX} · seeds=${SEEDS} · ${tasks.length} deals · workers=${WORKERS}/${logicalCpuCount()}\n`
);

async function main() {
  const started = Date.now();
  const rows = await runWorkerPool(path.join(__dirname, "temp-8x8-ge8-worker.js"), tasks, {
    workers: WORKERS
  });
  ...
  console.log("-".repeat(52));
  console.log(`OVERALL 8x8 G>=8: ${((100 * W) / D).toFixed(1)}% (${W}/${D})`);
  if (fails.length) {
    console.log(`\nFAILS (${fails.length}):`);
