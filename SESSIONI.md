...
**Meccanica:** tryDurissimaVitaExtra — rimette in tallone 1..tutte le carte (selective: tiene le meno "inquinanti"), mescola, pesca fino a mano N.
...
**API:** gnSoloBuildResidStats, gnSoloCardAxisProfile, gnSoloCellGeoDegree — usati in scoreSolo planner e fair finish.
...
**Aggiunto:** gnSoloFairDeepSearch — in coda (<=10 vuoti) DFS con posate da mano e pesca da tallone rimescolato come multiset (seed dal set uid).
...
(Prima bozza troppo aggressiva: 5 al 20% ma 6 collassato ad avg 6 — falsi negativi budget. Corretto.)
...
**Risultato: OBIETTIVO RAGGIUNTO** — 21/21 G<N + tutte G>=N/overcrowded.
...
## 2026-07-22 - Aggiornamento Manuale_Dura_Mater.docx

**File:** Dropbox `17 - DURA MATER\Manuale_Dura_Mater.docx` + copia `C:\Dev\Dura-Mater\Manuale_Dura_Mater.docx`
**Script:** `scripts/build-manuale-dura-mater.py` (Parte II riscritta)

**Aggiunte/correzioni:**
- Pesca Durissima = solo se posato + **refill** a mano iniziale
- Solitario: easy mode 2N e refill come **scelte giocatore**
- Divertimento / catene / Idea
- N reshuffle coop opzionale
- Formati e difficolta' solitario 3–8
- Riferimento rapido aggiornato (sezioni 15–25)

---

## 2026-07-22 - Terminologia regolamento (partita / mano / turno / giro)

**Decisione lessicale (per tutti i lettori, non gergo interno):**
- **Partita** = unita' di gioco completa (anche dentro il torneo: serie di partite)
- **Mano** = solo carte tenute; **mai** unita' di torneo (si evita il senso poker «hand = deal»)
- **Turno** = un giocatore; **Giro** = G turni (tavolo); **Torneo** / **Sessione** standard
- «Cedere la mano» → «cedere il turno»

**File aggiornati:** `Manuale_Dura_Mater.docx` (build + glossario Terminologia), `RULES.md`, `REGOLAMENTO-DURA-MATER.md`.

---

## 2026-07-22 - Manuale Durissima: carte scoperte, coordinatore, refill fisso

**Manuale / RULES:**
- Sez. 16: opzionali **carte scoperte** e **coordinatore** (coop)
- **Refill** = regola fissa; rimosso opt-out
- Easy mode 2N resta opzione solitario

**Risposta 8x8 0,2%:** mano **2N + refill** (+ path bot virtual-multi) — **senza** reshuffle.

---

## 2026-07-23 - Emulatore: easy mode 2N, mano bot, banner fine

- Easy mode: checkbox solitario only; extra = sempre **N** (mano 2N), non numero libero
- Solitario: nascosti giocatori / nome / config multi
- Mano computer scoperta (sidebar/dock) per seguire le scelte
- Board: chiavi assolute + no target a griglia piena (fix ultima carta)
- Banner grosso **Vittoria!** / **Fine partita** in gamebar (non copre carte)

---

## 2026-07-23 - Emulatore UI: tab sezioni, griglia, hub simulazioni

**Fix:** tab Competitivo/Collaborativo/Solitario (listener robusto + status feedback).  
**Griglia:** select 3x3…8x8 al posto di «lato matrice».  
**Torneo:** solo competitivo (scompare altrove).  
**Coordinatore:** rimosso dall'UI giocatore; sempre on in Durissima lato bot.  
**Easy/carte extra:** solo in Solitario (`#solo-options`).  
**Hub:** `simulazioni.html` (singolo + massivo); nav gioco → Gioco | Simulazioni; `index.html` → gioco.html.

---

## 2026-07-23 - Emulatore: rimosso salva/carica partita

**UI:** eliminati pulsanti Salva / Carica / Carica partita e input file JSON da `gioco.html` e `simulazione-singolo.html`.  
**JS:** rimossi `saveGame`, `loadGame`, `applyLoadedSession`, `readLoadedGameFile` e listener.  
**Restano:** undo/redo in partita; preferenze opzioni in `localStorage` (non file; non bloccano GitHub Pages).

Motivo: download/upload JSON e picker file inutili al prodotto e spesso problematici se si apre da ambienti ristretti; gioco = Avvia partita.

---

## 2026-07-23 - Emulatore browser: 3 sezioni + nomi sedi

**UI (`gioco.html`, `simulazione-singolo.html`, `game.js`):**
- Tab di sezione: **Competitivo** (Dura) / **Collaborativo** (Durissima G>=2) / **Solitario** (Durissima G=1)
- Allineato regole prodotto (refill fisso, no vita/reshuffle in UI)
- Nomi: sede 0 personalizzabile; 1–7 Dotto…Cucciolo; oltre l'ottavo **Nome2** (Eolo2, …) e **Nome3** se serve
- Prefs v3 con `playMode` (migrazione da variant)

Nav in alto (Gioco / Simulazione / Massivo) invariata.

---

## 2026-07-22 - N reshuffle / vita extra RIMOSSI dal prodotto

**Motivo:** stop di flusso non necessario; non usati nel bilanciamento solitario 2N+refill; su N bassi inutili.

**Codice:** `defaultDurissimaVitaExtraEnabled` sempre false; `isDurissimaVitaExtraEnabled` / `canUseDurissimaVitaExtra` hard-off. UI checkbox nascosta (`gioco.html`, `simulazione-singolo.html`). Implementazione legacy lasciata morta nel file (non cancellata per non far esplodere i probe storici).

**Docs:** sez. 23 reshuffle eliminata dal manuale (rinumerazione 23 formati, 24 rapido); RULES + AGENTS aggiornati.

**Test:** aspettative vita/reshuffle invertite (feature off).

---

## 2026-07-22 - Codice: rename torneo Hand → Game (partita)

**Motore / API (`mpcards-core.js`):**
- `tournamentHandIndex` → `tournamentGameIndex`
- `tournamentHandScores` → `tournamentGameScores`
- `tournamentHandLog` → `tournamentGameLog` (entry: `gameIndex`, `gameScores`, `starterWonGame`)
- `tournamentLastHandReason` → `tournamentLastGameReason`
- `tournamentHandsPlayed` / `tournamentMonteHands` → `...Games...`
- `beginNextTournamentHand` → `beginNextTournamentGame`
- `tournamentCompleteHand` → `tournamentCompleteGame`
- status `"hand_over"` → `"game_over"` (compat lettura legacy `hand_over`)
- `isTournamentGameOverStatus()` esposto

**Caller:** `game.js` (UI torneo **tenuta**, stringhe IT «Partita»), `simulator.js`, `tests/core-regression.test.js`, `scripts/tournament-audit*`, `tournament-sweep-lib.js`.

**Non toccato:** `state.hands` / mano carte UI; core frozen `.mpcards-core-legacy-neighbor.js` / `.mpcards-core-pre-jolly.js` (probe idea A/B storici).

**Non rifare:** rename inverso; non cancellare UI torneo in attesa di pass dedicato.

---

## 2026-07-22 - Bias posizione partita normale Dura (N=3..8, G legali 2..16)

**Obiettivo:** quanto la sede (ordine di partenza) influenza P(vincita) con bot attuale + inversioni 1° limite / Dura Mater attive.

**Setup:** partita singola competitiva (non torneo, non Durissima); `randomizeTurnOrder=false` (sedi fisse 0..G-1, S0 starter); strategia `planner` tutti; **20 partite/cella**; 52 celle legali (`isPlayableSetup`); 7 worker; ~42s.

**Script:** `scripts/seat-bias-probe.js` + `seat-bias-probe-worker.js`  
**Artefatti:** `results/seat-bias/seat-bias-N3-8-c20-planner-2026-07-22.{json,txt}`  
**Seed tag:** `seat-bias-v1-2026-07-22`

**Sintesi (spread = max-min win% tra sedi; starterDev = win% starter - 100/G):**

| Tipologia | Formati | Spread medio | Starter vs unif | chi2 sospetto (p~0.05) |
|-----------|---------|--------------|-----------------|------------------------|
| G=N | 6 | 44.6 pp | **+22.6 pp** | 3/6 |
| under (G<N) | 21 | 24.5 pp | +3.4 pp | 4/21 |
| over (G>N) | 25 | 27.6 pp | +1.2 pp | 14/25 (rumore alto con G grande e n=20) |

**G=N dettaglio:**
- 3x3: starter **95%** (unif 33%) · DM chiusa solo 15% · S0=19 S1=0 S2=1 — bias estremo
- 4x4: starter 55% (unif 25%) · DM 50%
- 5x5: starter 53% (unif 20%) · DM 53%
- 6x6 / 7x7 / 8x8: starter vicino a uniforme (+2..6 pp); 7x7 spread solo 10 pp

**Lettura inversioni:** dove la Dura Mater si chiude di rado (3x3 G=N, molti over), le due inversioni **non scattano abbastanza** e resta forte vantaggio di sede (starter su G=N piccoli; code/ultime sedi a volte su over). Dove DM chiude spesso (under tipico, G=N grandi) lo starter e' piu' vicino a 1/G.

**G=2:** S0 tipicamente 55–58% (mitigato); eccezioni 4x2 74% e 7x2 75% (campione piccolo).

**Caveat:** n=20 e' esplorativo; chi2 su G alti e' rumoroso (molte sedi a 0 vittorie per caso). Per conferma servirebbero 100–300/cella sui formati critici (3x3/4x4/5x5 G=N).
