...
- `Seed`: riproducibilità (derivato per ogni coppia LxG).
...
Per ogni partita con lato `L`, il sottomazzo usa le carte con **valore (1ª cifra) <= L** → `L  x  L` carte in partita.
...
4. **Dura Mater chiusa** — % partite con chiusura (ingombro LxL); % vittorie di chi ha chiuso (sulle partite chiuse); % stalli.
...
| `matrix-planner` / `matrix-random` / `matrix-hand-planner` | Tutte le celle LxG valide: successo vs stallo, **scalabilità con L** |
...
Menu **Workflow** in `simulator.html`: esegue più blocchi scenario in sequenza (es. sweep - + - + …) e produce un unico file JSON (`format: dura-mater-simulator-workflow-results`, versione 2) con un array `steps` — da incollare in chat invece di quattro run separate.
...
Sei workflow **`Matrice ` … `Matrice `** (`matrix-L3` … `matrix-L8`). Per ogni `L` fisso, due step:
...
In partita e nel motore vale sempre l'**inversione alla chiusura di ciascun limite della Dura Mater** (primo asse con fila/colonna di **N** carte, poi griglia **NxN**; due limiti nello stesso turno si annullano). Vedi `RULES.md`.
...
Un solo step: planner, ordine iniziale casuale, inversione ai limiti DM. Matrici ** … **, **G = 1 … L**. Circa **7 260** partite (220 per cella).
...
### Verifica «il gioco funziona» (matrici -)

| Cosa guardare | Competitiva (`L*-comp`) | Durissima (`L*-duri`, solo G=1) |
|---------------|-------------------------|----------------------------------|
| **success%** | Partite con **vincitore** | **Matrice LxL piena** |
